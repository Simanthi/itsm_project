...
const PurchaseRequestMemoForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<PurchaseRequestMemoData | PurchaseRequestMemoUpdateData>(initialFormData);
  const [displayData, setDisplayData] = useState<Partial<PurchaseRequestMemo>>({}); // For read-only fields in edit/view mode
  ...
  const fetchMemoForViewOrEdit = useCallback(async () => {
    ...
    setError(null);
    try {
      const memo = await getPurchaseRequestMemoById(authenticatedFetch, parseInt(memoId, 10));
      setFormData({ // Only set editable fields for the form
        item_description: memo.item_description,
        quantity: memo.quantity,
        reason: memo.reason,
        estimated_cost: memo.estimated_cost || null,
      });
      setDisplayData({ // Store all data for display purposes
        ...
        decision_date: memo.decision_date ? new Date(memo.decision_date).toLocaleDateString() : undefined,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.item_description || formData.quantity == null || formData.quantity <=0 || !formData.reason) {
    ...
    if (viewOnly && isEditMode) {
        showSnackbar("This request cannot be edited in its current state.", "info");
        return;
    }
    ...
    if (payload.estimated_cost != null && typeof payload.estimated_cost === 'string') {
    ...
    try {
      if (isEditMode && memoId) {
        await updatePurchaseRequestMemo(authenticatedFetch, parseInt(memoId, 10), payload as Partial<PurchaseRequestMemoUpdateData>);
  ...
  const readOnlyMode = viewOnly || (isEditMode && displayData.status !== 'pending');
  ...
  if (error && !isSubmitting) { // Show general page error if not submitting (submission errors shown inline)
    return (
        <Box sx={{ p:3 }}>
        ...
        <Grid container spacing={3}>
          ...
          {isEditMode && (
            <>
              ...
              <Grid item xs={12} sm={6}><Typography variant="body2"><strong>Status:</strong> {displayData.status || 'N/A'}</Typography></Grid>
              {displayData.approver_username && (
                <Grid item xs={12} sm={6}><Typography variant="body2"><strong>Approver:</strong> {displayData.approver_username}</Typography></Grid>
              )}
              {displayData.decision_date && (
                <Grid item xs={12} sm={6}><Typography variant="body2"><strong>Decision Date:</strong> {displayData.decision_date}</Typography></Grid>
              )}
              {displayData.approver_comments && (
                <Grid item xs={12}><Typography variant="body2"><strong>Approver Comments:</strong> {displayData.approver_comments}</Typography></Grid>
              )}
            </>
          )}
          ...
          <Grid item xs={12} sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
            ...
            {!viewOnly && (
              <Button type="submit" variant="contained" color="primary" disabled={isSubmitting || isLoading}>
                {isSubmitting ? <CircularProgress size={24} /> : (isEditMode ? 'Update Request' : 'Submit Request')}
              </Button>
            )}
          </Grid>
        </Grid>
