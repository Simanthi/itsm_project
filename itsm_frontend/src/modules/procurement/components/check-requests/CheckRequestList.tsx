...
const CheckRequestList: React.FC = () => {
  ...
  const [paymentDetails, setPaymentDetails] = useState<Partial<ConfirmPaymentPayload>>({
  ...
  }, [authenticatedFetch, page, rowsPerPage, sortConfigKey, sortConfigDirection, showSnackbar]);
  ...
  const handleSubmitForApproval = (id: number) => handleAction(() => submitCheckRequestForApproval(authenticatedFetch, id), 'Request submitted for approval!', 'Submission failed');
  const handleAccountsApprove = (id: number) => handleAction(() => approveCheckRequestByAccounts(authenticatedFetch, id, { comments: actionComments }), 'Request approved by accounts!', 'Approval failed');
  const handleAccountsReject = (id: number) => handleAction(() => rejectCheckRequestByAccounts(authenticatedFetch, id, { comments: actionComments || "Rejected by accounts." }), 'Request rejected by accounts!', 'Rejection failed');
  const handleMarkPaymentProcessing = (id: number) => handleAction(() => markCheckRequestPaymentProcessing(authenticatedFetch, id), 'Request marked as payment processing!', 'Update failed');
  const handleConfirmPaymentSubmit = (id: number) => handleAction(() => confirmCheckRequestPayment(authenticatedFetch, id, paymentDetails as ConfirmPaymentPayload), 'Payment confirmed!', 'Payment confirmation failed');
  const handleCancel = (id: number) => handleAction(() => cancelCheckRequest(authenticatedFetch, id), 'Request cancelled!', 'Cancellation failed');

  const openDialog = (req: CheckRequest, action: string) => {
    setSelectedRequest(req);
    setDialogAction(action);
    if (action === 'reject') setActionComments('');
    if (action === 'confirm_payment') setPaymentDetails({
        payment_method: 'check',
        payment_date: new Date().toISOString().split('T')[0],
        transaction_id: '',
        payment_notes: ''
  ...
  return (
    <Box sx={{ p: 3, width: '100%' }}>
          ...
          <TableBody>
            ...
            {checkRequests.map(req => {
              ...
              >
                ...
                <TableCell><Chip label={req.status.replace(/_/g, ' ')} color={getStatusChipColor(req.status)} size="small"/></TableCell>
                <TableCell>{new Date(req.request_date).toLocaleDateString()}</TableCell>
                <TableCell>{req.requested_by_username}</TableCell>
                <TableCell align="right">
                  <Tooltip title="View Details"><IconButton onClick={() => navigate(`/procurement/check-requests/view/${req.id}`)} size="small"><VisibilityIcon /></IconButton></Tooltip>
                  {req.status === 'pending_submission' && (req.requested_by === user?.id || isStaffUser) && (<Tooltip title="Edit"><IconButton onClick={() => navigate(`/procurement/check-requests/edit/${req.id}`)} size="small"><EditIcon /></IconButton></Tooltip>)}
                  {req.status === 'pending_submission' && (req.requested_by === user?.id || isStaffUser) && (<Tooltip title="Submit for Approval"><IconButton onClick={() => handleSubmitForApproval(req.id)} size="small" color="primary"><SendIcon /></IconButton></Tooltip>)}
                  {req.status === 'pending_approval' && isStaffUser && (<Tooltip title="Approve"><IconButton onClick={() => openDialog(req, 'approve')} size="small" color="success"><ThumbUpAltIcon /></IconButton></Tooltip>)}
                  {req.status === 'pending_approval' && isStaffUser && (<Tooltip title="Reject"><IconButton onClick={() => openDialog(req, 'reject')} size="small" color="error"><ThumbDownAltIcon /></IconButton></Tooltip>)}
                  {req.status === 'approved' && isStaffUser && (<Tooltip title="Mark Payment Processing"><IconButton onClick={() => handleMarkPaymentProcessing(req.id)} size="small" color="info"><PaymentsIcon /></IconButton></Tooltip>)}
                  {(req.status === 'approved' || req.status === 'payment_processing') && isStaffUser && (<Tooltip title="Confirm Payment"><IconButton onClick={() => openDialog(req, 'confirm_payment')} size="small" color="primary"><PriceCheckIcon /></IconButton></Tooltip>)}
      ...
      <Dialog open={!!dialogAction} onClose={closeDialog} maxWidth="sm" fullWidth>
        <DialogTitle>
          {dialogAction === 'reject' ? 'Reject Check Request' : dialogAction === 'approve' ? 'Approve Check Request' : 'Confirm Payment'}
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            {selectedRequest && `For Check Request CR-${selectedRequest.id} to ${selectedRequest.payee_name} for $${Number(selectedRequest.amount).toFixed(2)}.`}
            {(dialogAction === 'reject' || dialogAction === 'approve') && " Please provide comments for your decision."}
            {dialogAction === 'confirm_payment' && " Please provide payment details."}
          </DialogContentText>
          {(dialogAction === 'reject' || dialogAction === 'approve') && (
            <TextField autoFocus margin="dense" name="comments" label="Comments" type="text" fullWidth multiline rows={3} variant="outlined" value={actionComments} onChange={(e) => setActionComments(e.target.value)} />
          )}
            ...
            <Box sx={{mt: 2}}>
              <FormControl fullWidth margin="dense">
                ...
                <Select
                  ...
                  onChange={(e) => setPaymentDetails(prev => ({...prev, payment_method: e.target.value as PaymentMethod}))}
                >
                  {PAYMENT_METHOD_CHOICES.map(opt => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}
                </Select>
              </FormControl>
              <TextField margin="dense" name="payment_date" label="Payment Date" type="date" fullWidth variant="outlined" value={paymentDetails.payment_date} onChange={(e) => setPaymentDetails(prev => ({...prev, payment_date: e.target.value}))} InputLabelProps={{ shrink: true }} />
              <TextField margin="dense" name="transaction_id" label="Transaction ID / Check #" type="text" fullWidth variant="outlined" value={paymentDetails.transaction_id} onChange={(e) => setPaymentDetails(prev => ({...prev, transaction_id: e.target.value}))} required/>
              <TextField margin="dense" name="payment_notes" label="Payment Notes (Optional)" type="text" fullWidth multiline rows={2} variant="outlined" value={paymentDetails.payment_notes} onChange={(e) => setPaymentDetails(prev => ({...prev, payment_notes: e.target.value}))} />
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={closeDialog}>Cancel</Button>
          {dialogAction === 'reject' && <Button onClick={() => handleAccountsReject(selectedRequest!.id)} variant="contained" color="error">Confirm Rejection</Button>}
          {dialogAction === 'approve' && <Button onClick={() => handleAccountsApprove(selectedRequest!.id)} variant="contained" color="success">Confirm Approval</Button>}
          {dialogAction === 'confirm_payment' && <Button onClick={() => handleConfirmPaymentSubmit(selectedRequest!.id)} variant="contained" color="primary">Confirm Payment</Button>}
        </DialogActions>
      </Dialog>
    </Box>
  );
