...
import type { PurchaseOrder, PurchaseOrderStatus, OrderItem } from '../../../api/procurementApi';
...
const getStatusChipColor = (status: PurchaseOrderStatus) => {
    switch (status) {
      ...
      case 'invoiced': return 'secondary'; // Or some other color
...
const PurchaseOrderDetailView: React.FC = () => {
  ...
  const fetchPurchaseOrderDetails = useCallback(async () => {
    if (!poId || !authenticatedFetch) {
      setError("Purchase Order ID is missing or authentication is not available.");
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
    ...
    <Paper sx={{ p: { xs: 2, md: 4 }, m: { xs: 1, md: 2 } }} elevation={3}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          ...
          <Typography variant="body1"><strong>Order Date:</strong> {formatDateString(purchaseOrder.order_date)}</Typography>
          <Typography variant="body1"><strong>Expected Delivery:</strong> {formatDateString(purchaseOrder.expected_delivery_date)}</Typography>
        </Grid>
        <Grid item xs={12} md={6}>
          <Typography variant="h6" gutterBottom>Vendor & Creator</Typography>
          <Typography variant="body1"><strong>Vendor:</strong> {purchaseOrder.vendor_details?.name || 'N/A'}</Typography>
          <Typography variant="body1"><strong>Created By:</strong> {purchaseOrder.created_by_username || 'N/A'}</Typography>
          <Typography variant="body1"><strong>Created At:</strong> {formatDateString(purchaseOrder.created_at)}</Typography>
           {purchaseOrder.internal_office_memo && (
             <Typography variant="body1"><strong>Linked IOM ID:</strong> {purchaseOrder.internal_office_memo}</Typography>
           )}
        </Grid>
        ...
        {purchaseOrder.shipping_address && (
          <Grid item xs={12}>
            ...
            <Typography variant="body1" sx={{whiteSpace: 'pre-line'}}>{purchaseOrder.shipping_address}</Typography>
          </Grid>
        )}
            ...
            <Table size="small">
              ...
              <TableBody>
                {purchaseOrder.order_items.map((item: OrderItem) => (
                  <TableRow key={item.id || Math.random()}> {/* Use item.id if available, else fallback for new items not yet saved */}
        ...
        <Grid item xs={12} sx={{ textAlign: 'right' }}>
          <Typography variant="h6"><strong>Overall Total:</strong> {formatCurrency(purchaseOrder.total_amount)}</Typography>
        </Grid>
        ...
        {purchaseOrder.notes && (
          <>
            ...
            <Grid item xs={12}>
              ...
              <Typography variant="body1" sx={{whiteSpace: 'pre-line'}}>{purchaseOrder.notes}</Typography>
            </Grid>
          </>
        )}
      </Grid>
