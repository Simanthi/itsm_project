...
const CategoryPrintView: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading && !error && categoriesToPrint.length > 0 && autoPrint && printRootElement) {
  ...
  }, [loading, error, categoriesToPrint, autoPrint, navigate, printRootElement, location.pathname, state]);
  ...
  if (loading) { // Should be brief if no fetching
    return (
    ...
    <>
      ...
      >
        {categoriesToPrint.map((category) => (
          <Paper key={category.id} sx={{ p: { xs: 2, md: 3 }, pageBreakAfter: 'always' }} elevation={0} id={`category-print-${category.id}`}>
              ...
              <Grid item xs={12}>
                ...
                {category.description && (
                    <>
                        ...
                        <Typography sx={{ whiteSpace: 'pre-line' }}>{category.description}</Typography>
                    </>
                )}
              </Grid>
            </Grid>
