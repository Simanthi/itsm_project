...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const initializeAuth = async () => {
      ...
      if (storedToken && storedUser) {
        try {
          ...
          if (parsedUser && parsedUser.id !== undefined && parsedUser.id !== null && parsedUser.id !== 0) {
            ...
            console.error("AuthContext: Stored user data is invalid (missing or zero ID). Clearing.");
          ...
          setUser(null);
          setIsAuthenticated(false);
          console.log('AuthContext: Cleared invalid stored auth data due to parsing error.');
        }
      } else {
        ...
        setUser(null);
        setIsAuthenticated(false);
        console.log('AuthContext: No stored auth data. User is not authenticated.');
      }
  ...
  const login = useCallback(async (username: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      if (!response.user || response.user.id === undefined || response.user.id === null || response.user.id === 0) {
        console.error("AuthContext: Login API returned invalid user ID (undefined, null, or zero). Aborting login. User:", response.user);
      ...
      localStorage.removeItem('user');
      console.log('AuthContext: Login failed, cleared auth state in context and local storage.');
  ...
  const logout = useCallback(() => {
    ...
    localStorage.removeItem('authToken');
    localStorage.removeItem('user');
    console.log('AuthContext: Auth state cleared for logout in context and local storage.');
