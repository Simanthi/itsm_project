...
class ServiceRequestAdmin(admin.ModelAdmin):
    list_display = ('request_id', 'title', 'requested_by', 'assigned_to', 'status', 'category', 'priority', 'created_at')
    ...
    readonly_fields = ('request_id',) # Make sure the request_id is not editable in admin

@admin.register(ServiceRequestSequence)
class ServiceRequestSequenceAdmin(admin.ModelAdmin):
    list_display = ('current_alpha_part_char1', 'current_alpha_part_char2', 'current_numeric_part')
    # Make sure this model can only have one instance edited
    def has_add_permission(self, request):
        return not ServiceRequestSequence.objects.exists() # Only allow adding if no instance exists
